...
def encoding(dim, datatype, img, changed_ratio):
    # np.random.seed(0)
    ...
    print("grayscale_table shape:", len(grayscale_table), len(grayscale_table[4]))
    encoded_img = []
    for i in range(IMG_HEIGHT):
        row_encoded = []
        for j in range(IMG_WIDTH):
            pix_HV = np.multiply(grayscale_table[img[i][j]], pos[i][j])
